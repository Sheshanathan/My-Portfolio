...
export default function Navbar() {
  ...
  return (
    <header className={`navbar ${scrolled ? 'is-scrolled' : ''}`}>
      <nav className="desktop-nav" aria-label="Primary navigation">{navigation.map((item) => { const isActive = onHome && active === item.id; return <Link key={item.id} className={isActive ? 'active' : ''} aria-current={isActive ? 'location' : undefined} to={`/#${item.id}`} onClick={() => setOpen(false)}>{item.label}</Link> })}</nav>
